...
public class EmployeeWindow extends Stage {
	...
	private void initContent(GridPane pane) {
		...
		cbbCompany.getItems().addAll(Controller.getController().getCompanies());
	...
	private void okAction() {
		...
		if (name.length() == 0) {
			...
			if (wage < 0) {
				...
				if (employee != null) {
					Controller.getController().updateEmployee(employee, name, wage);
					if (companyIsSelected) {
						Controller.getController().addEmployeeToCompany(employee, newCompany);
					} else {
						Controller.getController().removeEmployeeFromCompany(employee, employee.getCompany());
					}
				} else {
					if (companyIsSelected) {
						Controller.getController().createEmployee(name, wage, newCompany);
					} else {
						Controller.getController().createEmployee(name, wage);
					}
				}
				hide();
			}
